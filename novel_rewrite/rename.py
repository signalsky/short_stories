...
def build_rename_prompt(novel_text: str) -> str:
    return (
        ...
        "2）每个人重新取名，并给出对应的新小名昵称，数量要和原aliases一致。\n"
        "3）必须补充人物关系，尤其母亲和子女、父亲和子女、兄弟姐妹。\n"
        "4）same_surname 字段表示该关系下是否应同姓。\n"
        "5）重命名时，same_surname=true 的关系要保持同姓。\n"
        "6）“夫人/先生/小姐”等属于称谓词，不要改称谓词本身，但可以按人物关系改姓，比如沈夫人可能需要改成林夫人。\n"
        "7）新标题要情绪强烈、女性读者偏好、有传播感。\n"
        "8）提取文中出现的关键时间点（如“5年”、“3个月”等），并建议修改后的时间。修改时必须考虑逻辑合理性（如孩子年龄、事件间隔等）。如果时间无需修改，new_time可与original_time相同。\n"
        "小说全文如下：\n"
        f"{novel_text}"
    )
