...
def run_scraper():
    try:
        ...
        with sync_playwright() as p:
            ...
            # Login
            print("请在打开的浏览器窗口中手动登录小红书账号。")
            page.goto("https://www.xiaohongshu.com/")
            
            # Check if logged in by looking for user avatar or specific element, or just wait for user
            print(">>> 登录成功后，请在此处按回车键继续 (或者等待30秒) <<<")
            try:
                input(">>> 登录成功后，请在此处按回车键继续 <<<")
            except EOFError:
                print("Input not supported, waiting 30 seconds for manual login...")
                time.sleep(30)
            ...
            for url in search_urls:
                ...
                for i, link_url in enumerate(collected_urls):
                    try:
                        ...
                        if success_count >= TARGET_COUNT:
                            print("Target of 5 items reached!")
                            break
